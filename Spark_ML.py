...
gbParamGrid = (ParamGridBuilder()
             .addGrid(gb.maxBins, [3, 5, 10])
             .addGrid(gb.maxDepth, [3, 5, 10])
...
lrCvPredictions = lrCvModel.bestModel.transform(testData)

display(lrCvPredictions.select("label", "prediction", "probability"))
...
gbCvPredictions = gbCvModel.bestModel.transform(testData)

display(gbCvPredictions.select("label", "prediction", "probability"))
...
transformed = lrCvModel.bestModel.transform(testData)

# Get the feature names from the DataFrame schema
feature_names = transformed.select("features").schema[0].metadata["ml_attr"]["attrs"]["binary"] + transformed.select("features").schema[0].metadata["ml_attr"]["attrs"]["numeric"]

# Retrieve the feature coefficients
feature_coefficients = lrCvModel.bestModel.coefficients
...
lrCvFeatureImportance = pd.DataFrame(data)

final_lr_df = lrCvFeatureImportance.sort_values(by=['feature_importance'],ascending =False)
...
# Get the feature names from the DataFrame schema
feature_names = rftransformed.select("features").schema[0].metadata["ml_attr"]["attrs"]["binary"] + rftransformed.select("features").schema[0].metadata["ml_attr"]["attrs"]["numeric"]

# Retrieve the feature coefficients
feature_coefficients = rfModel.coefficients
...
rfFeatureImportance = pd.DataFrame(data)

final_rf_df = rfFeatureImportance.sort_values(by=['feature_importance'],ascending =False)
pd.set_option('display.max_colwidth', None)
